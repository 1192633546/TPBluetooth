...
*/
    ...
    class ServerListener extends BleServerListener {
        ...
        public void onStartFailure(int errorCode) {
            ...
            //开始监听失败
            Log.e(TAG, "作为外围设备 开始监听失败:" + errorCode);

            switch (errorCode) {
                case ADVERTISE_FAILED_ALREADY_STARTED:
                    //已经开启了
                    Log.e(TAG, "失败,已经在监听了：" + errorCode);
                    mTvServerState.setText("开启服务器失败，已经在监听了");
                    break;
                case ADVERTISE_FAILED_DATA_TOO_LARGE:
                    //数据太大导致失败
                    Log.e(TAG, "服务器监听失败：数据太大" + errorCode);
                    mTvServerState.setText("开启服务器失败，广播数据太大");

                    break;
                case ADVERTISE_FAILED_FEATURE_UNSUPPORTED:
                    //准备开始时候失败
                    Log.e(TAG, "服务器监听失败：不支持" + errorCode);
                    mTvServerState.setText("开启服务器失败，不支持");

                    break;
                case ADVERTISE_FAILED_INTERNAL_ERROR:
                    //内部错误
                    Log.e(TAG, "服务器监听失败：内部错误" + errorCode);
                    mTvServerState.setText("开启服务器失败，内部错误");

                    break;
                case ADVERTISE_FAILED_TOO_MANY_ADVERTISERS:
                    //太多广播
                    Log.e(TAG, "服务器监听失败：太多服务器广播" + errorCode);
